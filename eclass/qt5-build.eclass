...
qt5_symlink_tools_to_build_dir() {
	...
	for tool in "${tools[@]}"; do
		[[ -e ${BROOT}${QT5_BINDIR}/${tool} ]] || continue
		ln -s "${BROOT}${QT5_BINDIR}/${tool}" . || die "failed to symlink ${tool}"
	done
...
qt5_base_configure() {
	...
	local conf=(
		...
		$([[ ${CHOST} != ${CTARGET} ]] && echo -device-option CROSS_COMPILE=${CHOST}-)
...
qt5_qmake() {
	...
	if [[ -n ${SYSROOT} ]] ; then
		...
		find -name "Makefile*" -exec sed \
			...
			-e "s@-I\(/usr/include/[^ ]* \)@-I${SYSROOT}\1@g" \
			-i {} +
	fi
}
