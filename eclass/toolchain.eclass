...
get_gcc_src_uri() {
	...
	[[ -n ${MPC_VER} ]] && \
		GCC_SRC_URI+=" https://www.multiprecision.org/mpc/download/mpc-${MPC_VER}.tar.gz"
	[[ -n ${MPFR_VER} ]] && \
		GCC_SRC_URI+=" https://www.mpfr.org/mpfr-${MPFR_VER}/mpfr-${MPFR_VER}.tar.xz"
	[[ -n ${GMP_VER} ]] && \
...
toolchain_src_prepare() {
	...
	[[ -n ${MPC_VER} ]] && \
		mv ${WORKDIR}/mpc-${MPC_VER} mpc
	[[ -n ${MPFR_VER} ]] && \
		mv ${WORKDIR}/mpfr-${MPFR_VER} mpfr
	[[ -n ${GMP_VER} ]] && \
		mv ${WORKDIR}/gmp-${GMP_VER} gmp
